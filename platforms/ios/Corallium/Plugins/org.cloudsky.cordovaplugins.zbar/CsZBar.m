...
- (void)scan: (CDVInvokedUrlCommand*)command;
{
    if(self.scanInProgress) {
        ...
        if(drawSight){
            ...
            CGFloat dim = screenWidth < screenHeight ? screenWidth / 2.1 : screenHeight / 2.1;
            ...
            UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(dim / 2, dim/4, 1, dim/2)];
            lineView.backgroundColor = [UIColor yellowColor];
            [polygonView addSubview:lineView];

            UIView *lineView2 = [[UIView alloc] initWithFrame:CGRectMake(dim/4, dim/2, dim/2, 1)];
